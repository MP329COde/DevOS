...
- [x] Étendre le panel HAProxy (Phase 8 déjà fait côté backend) avec édition guidée frontend des frontends/ACL/certificats déjà exposés par l'API, pas seulement lecture de backends/serveurs — `HAProxyClient` étendu avec `listAcls`/`addAcl`/`deleteAcl` (Data Plane API v3 `/configuration/acl`, verrouillage optimiste par version comme pour les serveurs) et `listCertificates` (lecture seule, `/storage/ssl_certificates`) ; panel HAProxy affiche désormais les frontends avec leurs règles ACL (ajout/suppression) et une section certificats TLS
- [x] Ajouter des actions de contrôle VM Proxmox (start/stop/reboot) côté backend (`backend/src/catalog/proxmox.ts` déjà en lecture seule — étendre avec confirmation explicite avant toute action destructive, cohérent avec l'identité visuelle "infra critique" de `Design.md`) et panel frontend dédié — `ProxmoxClient.controlVirtualMachine`, endpoint `POST /api/proxmox/nodes/:node/vms/:vmid/:action` (rôle `execute_infrastructure` + `confirm: true` obligatoire côté serveur, 409 sinon), nouveau panel `frontend/src/components/ProxmoxPanel.tsx` avec overlay de confirmation explicite (fond sombre, accent terracotta, identique à l'identité visuelle du module Réseau) avant tout démarrage/arrêt/redémarrage
- [x] Vérifier Playwright : actions VM avec confirmation, affichage HAProxy étendu — vérifié en conditions réelles (backend+frontend démarrés, 503 propre sans HAProxy/Proxmox configurés) puis avec données mock : overlay de confirmation affiché au clic sur une action VM, "Annuler" referme sans appel réseau, "Confirmer" déclenche l'appel ; ajout d'une règle ACL sur un frontend HAProxy avec apparition immédiate dans la liste, aucune erreur console/réseau inattendue
