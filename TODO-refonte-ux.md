...
- [ ] Remplacer les boutons monter/descendre par un vrai drag-and-drop (pointer events natifs, pas de nouvelle dépendance lourde type dnd-kit sauf si strictement nécessaire — justifier le choix si une lib est ajoutée)
- [ ] Rendre le mode édition visuellement évident sur toute la grille (pas seulement sur chaque carte isolée) : bordure en pointillés sur `.widget-grid`, léger fond distinct
- [ ] Ajouter un panneau "Ajouter un widget" listant les widgets masqués, plutôt que de devoir deviner qu'il faut recliquer sur le `×` d'une carte grisée
