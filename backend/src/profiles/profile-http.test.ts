...
import { handleProfileRequest, type ProfileHttpService } from './profile-http.js';
...
test('POST /api/roles requires a name', async () => {
  const result = await handleProfileRequest('POST', '/api/roles', {}, service());
  assert.equal(result.status, 400);
});

test('POST /api/roles creates a role', async () => {
  const result = await handleProfileRequest('POST', '/api/roles', { name: 'Réseau', permissions: ['read'] }, service());
  assert.equal(result.status, 201);
});

test('POST /api/profiles requires email and displayName', async () => {
  const result = await handleProfileRequest('POST', '/api/profiles', { displayName: 'Alice' }, service());
  assert.equal(result.status, 400);
});

test('GET /api/profiles?email= routes to getProfileByEmail', async () => {
  const result = await handleProfileRequest('GET', '/api/profiles?email=a@b.c', undefined, service({
...
test('PUT /api/dev-projects/:id/permissions sets a project permission', async () => {
  const result = await handleProfileRequest('PUT', '/api/dev-projects/proj1/permissions', { userProfileId: 'u1', roleId: 'r1' }, service());
  assert.equal(result.status, 200);
});

test('PUT /api/dev-projects/:id/permissions rejects missing fields', async () => {
  const result = await handleProfileRequest('PUT', '/api/dev-projects/proj1/permissions', {}, service());
  assert.equal(result.status, 400);
});

test('DELETE /api/dev-projects/:id/permissions/:userProfileId removes it', async () => {
  const result = await handleProfileRequest('DELETE', '/api/dev-projects/proj1/permissions/u1', undefined, service());
  assert.equal(result.status, 204);
});

test('unknown route returns 404', async () => {
  const result = await handleProfileRequest('GET', '/api/unknown', undefined, service());
